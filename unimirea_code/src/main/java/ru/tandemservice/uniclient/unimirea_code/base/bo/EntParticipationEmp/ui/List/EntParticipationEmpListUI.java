...
public class EntParticipationEmpListUI extends UIPresenter {
    ...
    {
        if (null != _employeeID) {
            Employee employee = DataAccessServices.dao().get(getEmployeeID());
            getSettings().set(EntParticipationDSHandler.FIRST_NAME , employee.getPerson().getIdentityCard().getFirstName());
            getSettings().set(EntParticipationDSHandler.LAST_NAME , employee.getPerson().getIdentityCard().getLastName());
            //employee.get
        }
